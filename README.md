...
このプロジェクトは、ローカル向けモデルだけで完結する会話バックエンドを作るための実験でもあります。

開発時の実感として、Qwen3.6-27B は複雑な判断や構成を考える力が強い一方、日本語の自然な会話文では少し硬さや不自然さが出ることがあります。逆に Gemma4 系列は E2B のような小型モデルでも日本語の会話文が自然に出やすい一方、細かい条件整理や長めの思考では弱さが出ることがあります。

そのため、このGatewayでは「Qwenの思考力で会話の方針や構成を支え、Gemma4の自然な日本語でユーザーに返す」ことを試しやすいように、Router / Actor / Director / Formatter を分けています。たとえば Director に大きめの推論モデル、Actor や Formatter に自然な日本語が得意なモデルを割り当てることで、1つのモデルだけに全役割を任せない構成を取れます。
...
app/
  ...
  memories/   Example static character memory files
...
`character_memory_paths` はキャラクター本人の静的な記憶YAMLです。展示カタログとは別に、user messageと `id` / `keywords` / memory本文を照合し、関連する記憶だけをcompanion contextへ追加します。一致しない記憶は注入しません。
